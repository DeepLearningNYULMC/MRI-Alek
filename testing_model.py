...
def main(_):
  ...
  with tf.Session(config = config) as sess:
    ...
    for i in range(len(test_x2)):
      temp3 = accuracy.eval(feed_dict={x: test_x2[i:i+1, :], y_: test_y2[i:i+1, :], keep_prob: 1.0})
      print('test accuracy %g' % temp3)
      total_accuracy = total_accuracy + temp3
      total_times = total_times+1
      print("predictions", prediction.eval(feed_dict={x: test_x2[i:i+1, :], keep_prob: 1.0}, session=sess))
      probability = probabilities.eval(feed_dict={x: test_x2[i:i+1, :], keep_prob: 1.0}, session=sess)
      print(probability.ravel())
      if i==0:
        probs_array = probability.ravel()
        continue
      probs_array = numpy.vstack([probs_array, probability.ravel()])
    ...
    roc_auc = dict()
    for i in range(3):
      fpr[i], tpr[i], _ = roc_curve(test_y2[:, i], probs_array[:, i])
      roc_auc[i] = auc(fpr[i], tpr[i])

# Compute micro-average ROC curve and ROC area
    fpr["micro"], tpr["micro"], _ = roc_curve(test_y2.ravel(), probs_array.ravel())
